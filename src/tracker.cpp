...
int main(int argc, char **argv)
{
    ...
    nt::StartClientTeam(nt::GetDefaultInstance(), 4795, 5801);
    ...
    while (true)
    {
        ...
        Canny(thresholdFrame, contourFrame, 10, 10 * 3, 3);
        ...
        //if we have a contour that satisfies requirements, draw a green outline around it and send its features over the network
        if (latestIndex != -1)
        {
            drawContours(rawFrame, initContours, latestIndex, Scalar(0, 255, 0), 3);
            Moments M = moments(initContours[latestIndex]);
